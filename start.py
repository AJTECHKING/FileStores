...
async def start_command(client: Client, message: Message):
    ...
    if len(text)>7:
        try:
            ...
            return
        try:
            await client.get_chat_member(REQUEST_CHANNEL_ID, message.from_user.id)
                ...
                if message.from_user.id not in users_ids:
                    buttons = [[
                        InlineKeyboardButton("Request Join Channel", url=REQUEST_CHANNEL_LINK)
                    ],[
                        InlineKeyboardButton("Try Again", url=f"https://t.me/{client.username}?start={base64_string}")
                    ]]
                    return await message.reply(REQUEST_JOIN_MSG, reply_markup=InlineKeyboardMarkup(buttons), quote=True)
